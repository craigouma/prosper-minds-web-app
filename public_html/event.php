 ...
 * One school, in full.
 *
 * Rebuilt in Phase 3 on the design system. The page it replaces was the last
 * public page still served by assets/css/style.css, and it carried a Google
 * Fonts link, a Font Awesome CDN stylesheet and a third-party QR code image
 * generated by api.qrserver.com, which meant every visit to a course page made
 * three cross-border requests to hosts nobody had reviewed. None of them
 * survive here.
 *
 * WHAT COMES FROM WHERE
 * ---------------------
 * Every FACT about the school comes from the `events` row: title, dates,
 * location, prices, agenda, audience, the three perk lists. Nothing about a
 * school is written into this file, because there are four of them and there
 * will be more.
 *
 * Every LABEL around those facts comes from page_content slug 'event', so the
 * client can retitle a section without a deploy. Each call passes the same
 * words inline as its default, so an unreachable content table costs the page
 * nothing.
 *
 * The early bird position is COMPUTED on every render from the three tier
 * columns, never seeded. Once all three have lapsed the page degrades to a
 * non-dated line rather than printing a deadline that has passed.
 *
 * WHAT A MISSING EVENT DOES
 * -------------------------
 * The page it replaces answered an unknown id with a 302 to the homepage, which
 * tells a search engine the URL moved and tells a visitor nothing about why
 * they are suddenly somewhere else. This one answers 404 with a page that says
 * what happened and links to the calendar.
 *
 * A finished school stays reachable even after an admin unpublishes it: the
 * client's instruction is that past events are never hidden. An UNPUBLISHED
 * FUTURE event is a draft and answers 404. That rule is pmEventIsListable().
 *
 * House style: no em dashes in any user-visible copy. Client instruction, and
 * the `events` rows themselves break it in nine places, which is what
 * pmEventProse() is for.
 */
