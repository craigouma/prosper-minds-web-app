...
> **Revision 2 note, if you're pasting this as a follow-up into a session that already produced a first version:** three changes from that first pass — (1) switch the dominant theme from dark to **light**, per the audience note in Section 2; (2) the existing event cards' designed banner-image tiles (see Section 3) should stay a first-class element, not get replaced with generic photography; (3) the Events page (Section 6, item 2) now needs an added **banner gallery section** showing every event's current promotional banner in one place. Everything else in this brief still stands.
>
> **Revision 3 note:** the current live site has a **Sponsorship page that was missed from the original page list** — it's real, already has substantial content, and needs to be added as page 9 in Section 6. See the new Section 3 content below and the new item in Section 6.
>
> **Revision 4 note:** add a **newsletter signup field to the site-wide footer** (email input + submit, matches the current live site) — deliberately **not a popup/modal**, which conflicts with the brand's "never playful, strategic not sales-heavy" tone. See the new line in Section 6.
...
**Imagery and graphic style:** Clean, minimal, intentional. Strong contrast. Line-based, rounded, minimal iconography. Neural-network / node / abstract mind-inspired patterns encouraged as background texture, used sparingly and with intent, not as filler. Shield and head elements may appear subtly as background motifs.

**Theme: light, not dark — this is a deliberate departure from the brand PDF's own visual-style note ("dark backgrounds with green highlights"), made for a real audience reason.** Delegates skew older and mid-career (accountants, auditors, treasury leaders, government officials — not a young startup crowd), and that audience reads dark UI as harder to use, not as premium. So: **white/light-neutral as the dominant background across the site**, with black used for strong text and high-contrast blocks (footer, section dividers, select CTA bands) and green as the accent/action color (buttons, highlights, links, stats) — not the other way around. Dark sections should be used deliberately and sparingly (e.g. a footer, one bold CTA band) as punctuation, not as the primary surface. Keep the boldness and confidence of the brand's own marketing posters (strong typographic hierarchy, high contrast, declarative short copy like "WHEN PRESSURE HITS, LEADERS RISE") — just realized on light backgrounds instead of dark ones. Do not default to soft pastel-green washes everywhere either (that's the current live site's problem, see Section 4) — light does not mean low-contrast or airy; keep it crisp: white/near-white surfaces, black text, decisive green accents, real contrast between elements.
...
**Existing asset worth keeping: designed event banner tiles.** Each event card on the current homepage already displays a real, purpose-designed promotional banner image (not a generic photo) — e.g. a "Future-Ready PFM Leaders" banner with the course title, dates, price, and a call-to-action baked into the graphic itself. Prosperminds' design team actively produces these per event (this is also what Priority 4 of the client's onboarding email refers to — keeping banners current and posting them to LinkedIn). **These banner images are a real, valuable, already-existing asset — feature them prominently as the visual anchor of each event card/listing, don't replace them with generic stock photography or illustration.**
...
**Sponsorship program** (a real, separate page on the live site, missed from the original version of this brief — this content already exists and is more confident/bold than the rest of the current site, keep the energy, not just the words): positioned as "Co-Author Africa's Public Finance Future" — B2G (business-to-government) partnership for organizations wanting direct access to the senior public-sector audience at these events, not generic ad placement. Covers **all four flagship events** (Cape Town Oct, Kuala Lumpur Nov, Bali Dec, Mombasa Dec — the live site's current version only lists the first three; Mombasa is being added as part of this redesign). Four sponsorship tiers (Platinum $15,000, Gold $7,500, Silver, Bronze — each with a slot-availability count and a distinct benefits list: speaking slots, VIP passes, branding placement, exhibition space, press features), three **specialised $1,000 entry packages** (Gala Dinner, Digital Experience, Exhibitor), and five **$500 add-ons** (Lanyard, Delegate Bag, Wi-Fi, Mobile App, Water Station sponsorship). Ends in a real enquiry form (organization, contact details, which event(s), tier of interest, goals/message) — "we do not sell packages, we build partnerships tailored to your goals," 48-hour response promise.
...
3. **Visual identity lacks confidence and consistency.** The live site leans on soft pastel-green washes, plain white cards, and generic corporate stock photography (people pointing at laptops, generic handshake photos) that could belong to any company — light in the wrong way: low-contrast and generic rather than crisp and deliberate. One dark section exists (a stats block) and reads as a random interruption rather than part of a system — there's no consistent visual rhythm. The fix isn't "make it dark" (see the light-theme direction in Section 2) — it's real contrast, real typographic hierarchy, and one consistent system, not soft pastels everywhere.
...
If in doubt on any specific element, the test is: **does this look as bold, confident, and high-contrast as the brand's own marketing posters — just on light backgrounds instead of dark (Section 2) — or does it look like the default output of a generic AI page builder?** If it's the latter, don't use it.
...
1. **Homepage** — hero, the four flagship events (as a real preview grid featuring each event's actual designed banner tile, linking to real event pages — not everything crammed in), the three service pillars, credibility stats (25 years / 875 leaders trained — real numbers, keep them), a fixed (not broken) testimonial section, footer.
2. **Events listing / CPD calendar page** — all events, filterable by upcoming vs. past, each linking to its own detail page, each shown via its real designed banner tile (see Section 3). This is a real, separate page, not a homepage section. **This page also needs a banner gallery section** — a clear area showing every current event's promotional banner together in one place (grid of banner images, downloadable/shareable), since Prosperminds' design team produces these per event and needs a central place to reference and pull from for social/LinkedIn posting.
...
8. **Sponsorship page** — the B2G partnership pitch described in Section 3: value proposition, all four eligible events, the four-tier package matrix (Platinum/Gold/Silver/Bronze) plus the specialised $1,000 packages and $500 add-ons, and the enquiry form. Real, existing content — carry the confident/bold energy of the current version into the new light-theme system, don't water it down to match the more generic homepage.
9. **404 / not-found state** — on-brand, not a generic error page.

**Site-wide footer** (every page): standard footer content plus a **newsletter signup field** — a simple email input + submit button, matching what the current live site already has. Deliberately footer-only, **not a popup or modal** — see Revision 4 note at the top for why.

For each page: show **both desktop and mobile**, and keep the light theme consistent throughout (per Section 2) — any black/dark blocks should be deliberate, sparing accents (footer, one CTA band), not alternating with light sections at random.
