 ...
 *
 * One token per session (not per request), so a visitor can register for more
 * than one event, or retry after a validation error, without the token going
 * stale under them.
 *
 * Usage:
...
/**
 * Start the session if one is not already running. Must be called before any
 * output, otherwise PHP cannot send the session cookie.
 */
